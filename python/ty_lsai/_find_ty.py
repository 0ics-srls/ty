...
def find_ty_bin() -> str:
    ...
    targets = [
        ...
        (
            # On Windows, with module path `<prefix>/Lib/site-packages/ty_lsai`
            _join(_matching_parents(_module_path(), "Lib/site-packages/ty_lsai"), "Scripts")
            if sys.platform == "win32"
            # On Unix,  with module path `<prefix>/lib/python3.13/site-packages/ty_lsai`
            else _join(
                _matching_parents(_module_path(), "lib/python*/site-packages/ty_lsai"),
                "bin",
            )
        ),
        # Adjacent to the package root, e.g., from `pip install --target`
        # with module path `<target>/ty`
        _join(_matching_parents(_module_path(), "ty_lsai"), "bin"),
